...
namespace Ecommerce.Infrastructure.Repository
{
    public class CartItemRepository(EcommerceContext context) : ICartItemRepo
    {
        ...
        {
            return await _context.GetCartItemsForUser(filteringOptions.UserId).Include(ci => ci.Product).OrderBy(c => c.Id).ToListAsync();
        }

        public async Task<CartItem> GetByIdAsync(int id)
        {
            var result = await _context.GetCartItemById(id)
